...
### 10.4.1 Sanity provider (v1.2 pilot, Task 116 + Task 117)
...
**Sanity Studio (Task 117).** The Sanity Studio is shipped under `studio/` at the repo root as a separate npm project. The Studio defines the three document schemas (`property`, `agent`, `development`) the editor uses to populate the dataset. The Studio schema is the **editor contract**; the Nuxt Zod schema is the **runtime contract**. The two are matched by hand — the Studio schema's field names, enum values, and reference types match the GROQ projection in `server/utils/sanity-mappings.ts` so the boundary schema validation passes. The Studio's schema tests (23 cases) verify the field names match the GROQ projection and the enum values match the runtime Zod schemas. The Studio is deployed to Sanity's managed hosting with `cd studio && pnpm deploy` and is accessed at `<projectId>.sanity.studio`. The agency owns the Sanity project and revokes the implementer's access after the handoff. See `studio/README.md` for the full setup / deploy / access workflow.
