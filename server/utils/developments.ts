 ...
 * static / api / cms source selection, and exposes the resolved
 ...
 *    developments service calls via `$fetch`) and by the
 *    sitemap at `server/routes/sitemap.xml.ts` (which imports
 *    the loader directly without going through the Nitro
 *    endpoint).
 *  - {@link createDevelopmentsServerAdapter} — lower-level
 ...
 * cms-driver modules and the five `NUXT_DEVELOPMENTS_*` env
 * vars therefore cannot reach the client bundle by code
 ...
 * schema, the bundled `sampleDevelopments` catalog, and the
 * CMS branch's `createHttpJsonCmsDriver` +
 * `createCmsDataSource` wiring.
 *
 * **CMS support (Task 110).** The developments loader now ships
 * a CMS branch that mirrors the agents CMS path (v1.1.0 M26)
 * and the property CMS path (v1.1.0 M20):
 * `NUXT_DEVELOPMENTS_DATA_SOURCE=cms` +
 * `NUXT_DEVELOPMENTS_CMS_URL` selects the simple HTTP/JSON CMS
 * driver + `developmentListSchema` boundary. The
 * provider-driver boundary is unchanged — the shipped driver
 * is the same `createHttpJsonCmsDriver` the property and agents
 * CMS paths use, and the adapter is the same
 ...
 * — so the api / cms fetch happens on demand, not on every
 * request.
 *
 * **No permanent process-lifetime cache.** A successful
 * API / CMS result is NOT retained between calls. Each call to
 * {@link loadDevelopmentsServer} constructs a fresh adapter
 * and awaits its `loadAll()`; a later request observes the
 * latest upstream data, not a stale snapshot. The only shared
 * state is the `pending` reference captured in the per-feature
 ...
 * `NUXT_DEVELOPMENTS_API_URL`, `NUXT_DEVELOPMENTS_API_TIMEOUT_MS`,
 * `NUXT_DEVELOPMENTS_CMS_URL`, and `NUXT_DEVELOPMENTS_CMS_TIMEOUT_MS`
 ...
 *  - `NUXT_DEVELOPMENTS_DATA_SOURCE=cms` with an empty /
 *    whitespace `NUXT_DEVELOPMENTS_CMS_URL` raises
 *    {@link DataSourceMissingConfigError}; with a non-empty
 *    endpoint, the loader constructs the CMS adapter (the
 *    simple HTTP/JSON driver + `developmentListSchema`
 *    boundary).
 ...
 *  - A non-integer `NUXT_DEVELOPMENTS_API_TIMEOUT_MS` or
 *    `NUXT_DEVELOPMENTS_CMS_TIMEOUT_MS` falls back to the
...
const PROP_ENV_CMS_URL = 'NUXT_DEVELOPMENTS_CMS_URL'
 ...
 *
 * The `cms` branch (Task 110) supplies the developments' CMS
 * configuration: the endpoint / timeout env-var names, the
 * boundary Zod schema (the same `developmentListSchema` the
 * api + static paths use), and the `build` callback that wires
 * `createHttpJsonCmsDriver` + `createCmsDataSource` —
 * identical to the property + agents CMS paths, scoped to the
 ...
 * `createDevelopmentsServerAdapter()` call site (the property
 * and agents loaders follow the same pattern).
 *
 *  - `NUXT_DEVELOPMENTS_DATA_SOURCE` unset / empty / whitespace
 *    / `'static'` → the bundled static adapter.
 *  - `NUXT_DEVELOPMENTS_DATA_SOURCE=api` + a non-empty
 *    `NUXT_DEVELOPMENTS_API_URL` → the api adapter.
 *  - `NUXT_DEVELOPMENTS_DATA_SOURCE=cms` + a non-empty
 *    `NUXT_DEVELOPMENTS_CMS_URL` → the CMS adapter (Task 110).
 *  - `NUXT_DEVELOPMENTS_DATA_SOURCE=cms` + an empty /
